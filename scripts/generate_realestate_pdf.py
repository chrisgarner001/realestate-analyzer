...
def _money(value):
    """Format numeric values into money strings."""
    if value is None:
        return ""
    try:
        number = float(value)
...
def normalize_report_data(data):
    """Normalize both legacy and current report JSON schemas into the shape expected by the renderer."""
    ...
    address = (
        ...
        or data.get("address")
        or "123 Main Street, Austin, TX 78701"
    )
    price = data.get("listing_price") or data.get("asking_price") or data.get("price") or "$425,000"
    date_value = data.get("report_date") or data.get("date") or datetime.now().strftime("%B %d, %Y")
    overall_score = (
        data.get("property_score")
        if data.get("property_score") is not None
        else data.get("overall_score", 0)
    )

    normalized["address"] = address
    normalized["price"] = _money(price) if not isinstance(price, str) else price
    normalized["date"] = date_value
    normalized["overall_score"] = float(overall_score) if overall_score not in (None, "") else 0

    property_details = dict(data.get("property_details") or {})
    property_details.setdefault("beds", data.get("beds") or "3")
    property_details.setdefault("baths", data.get("baths") or "2")
    property_details.setdefault("sqft", data.get("sqft") or "1,850")
    property_details.setdefault("year_built", data.get("year_built") or "2005")
    property_details.setdefault("lot_size", data.get("lot_size") or "0.18 acres")
    property_details.setdefault("property_type", data.get("property_type") or "Single Family Residence")
    normalized["property_details"] = property_details

    comps = data.get("comparable_sales") or data.get("comps") or []
    normalized_comps = []
    for item in comps:
        if isinstance(item, dict):
            normalized_item = {
                "address": item.get("address") or item.get("property_address") or "",
                "price": item.get("price") or item.get("sale_price") or "",
                "sqft": item.get("sqft") or item.get("square_feet") or "",
                "price_sqft": item.get("price_sqft") or item.get("psf") or "",
                "sold_date": item.get("sale_date") or item.get("sold_date") or item.get("date") or "",
                "distance": item.get("distance") or "",
            }
            normalized_comps.append(normalized_item)
    normalized["comps"] = normalized_comps or data.get("comps") or []

    if isinstance(data.get("categories"), dict):
        normalized["categories"] = data.get("categories")

    cashflow = dict(data.get("cashflow") or {})
    if not cashflow.get("items"):
        est_rent = data.get("estimated_rent") or data.get("estimated_monthly_rent")
        net_cash = data.get("net_cash_flow") or data.get("monthly_cash_flow")
        cap_rate = data.get("cap_rate")
        cash_on_cash = data.get("cash_on_cash")
        grm = data.get("gross_rent_multiplier")
        vacancy_rate = data.get("vacancy_rate")
        cashflow["items"] = [
            {"item": "Gross Rental Income", "monthly": _money(est_rent), "annual": _money((est_rent or 0) * 12)},
            {"item": "Vacancy Loss", "monthly": f"-${_money((est_rent or 0) * (vacancy_rate or 0) / 100) if vacancy_rate else '$0'}", "annual": ""},
            {"item": "Net Cash Flow", "monthly": _money(net_cash), "annual": _money((net_cash or 0) * 12)},
        ]
    normalized["cashflow"] = cashflow

    investment_metrics = dict(data.get("investment_metrics") or {})
    if "cap_rate" not in investment_metrics and data.get("cap_rate") is not None:
        investment_metrics["cap_rate"] = _pct(data.get("cap_rate"))
    if "cash_on_cash" not in investment_metrics and data.get("cash_on_cash") is not None:
        investment_metrics["cash_on_cash"] = _pct(data.get("cash_on_cash"))
    if "grm" not in investment_metrics and data.get("gross_rent_multiplier") is not None:
        investment_metrics["grm"] = f"{float(data.get('gross_rent_multiplier')):.1f}x"
    normalized["investment_metrics"] = investment_metrics

    mortgage = dict(data.get("mortgage") or {})
    mortgage.setdefault("purchase_price", normalized["price"])
    mortgage.setdefault("down_payment", data.get("down_payment") or "$85,000 (20%)")
    mortgage.setdefault("loan_amount", data.get("loan_amount") or "$340,000")
    mortgage.setdefault("rate", data.get("rate") or "6.75%")
    mortgage.setdefault("term", data.get("term") or "30-year fixed")
    mortgage.setdefault("monthly_pi", data.get("monthly_pi") or "$2,205")
    mortgage.setdefault("monthly_piti", data.get("monthly_piti") or "$2,684")
    normalized["mortgage"] = mortgage

    neighborhood = dict(data.get("neighborhood") or {})
    if not neighborhood.get("scores") and data.get("scores"):
        neighborhood["scores"] = data.get("scores")
    normalized["neighborhood"] = neighborhood

    recommendation = dict(data.get("recommendation") or {})
    recommendation.setdefault("signal", property_signal(normalized["overall_score"]))
    recommendation.setdefault("summary", "This property is a viable investment opportunity with a balanced risk profile and favorable long-term upside.")
    recommendation.setdefault("suggested_offer", data.get("suggested_offer") or "$405,000 - $415,000")
    normalized["recommendation"] = recommendation

    risk_factors = data.get("risk_factors") or []
    normalized["risk_factors"] = risk_factors
    normalized["strategies"] = data.get("strategies") or []
    normalized["appreciation_projections"] = data.get("appreciation_projections") or []
    normalized["scenarios"] = data.get("scenarios") or []
    normalized["comp_summary"] = data.get("comp_summary") or {
